#A랑 내용은 같다 단지 서브넷을 다르게 만들어준다.
resource "aws_instance" "api-b" {
  ...
  vpc_security_group_ids = [
    ...
    aws_security_group.test-default1.id      # default1은 만들지 않았지만 대충 이렇게 쓴다는걸 보여줄려고 씀
  ]
  subnet_id = aws_subnet.test-b.id           #무슨 서브넷에 만들래?
